...
* Keep track\* off every `Govern` ⟷ `GovernQueue` pair assigning it a name on the blockchain. This means that instead of assigning an ENS name, this is left to the user, as the name itself will be saved in the contract's storage.
* Setting metadata, which means you can include an IPFS CID (Content Identifier) to talk about your DAO, and include relevant links.
